...
%Testing Image Patches -------------------------------
%directory to save patches too 
folder = '/Users/brittaneyeveritt/Documents/GitHub/Deep-Learning-Project/TestPatchesOrdered/';

%Load the masks into MATLAB
path = '/Users/brittaneyeveritt/Documents/GitHub/Deep-Learning-Project/TestingNucleiMasks/*.png';
path2 = '/Users/brittaneyeveritt/Documents/GitHub/Deep-Learning-Project/TestingNucleiMasks/';
images = dir(path);      
        ...
        for col = 1:15:i 
            ...
            if (patch_i == 75 & patch_j == 75)
                if k==1 | k==2 | k==3 | k==4 | k==5 | k==6 | k==7 | k==8 | k==9
                    if count < 10
                        filename = fullfile(folder,sprintf('%s%d_%s%d%s','test_patch_0',k,'000',count,'.png'));
                    elseif count >= 10 & count < 100
                        filename = fullfile(folder,sprintf('%s%d_%s%d%s','test_patch_0',k,'00',count,'.png'));
                    elseif count >= 100 & count < 1000
                        filename = fullfile(folder,sprintf('%s%d_%s%d%s','test_patch_0',k,'0',count,'.png'));
                    else
                        filename = fullfile(folder,sprintf('%s%d_%d%s','test_patch_0',k,count,'.png'));
                    end
                else
                    if count < 10
                        filename = fullfile(folder,sprintf('%s%d_%s%d%s','test_patch_',k,'000',count,'.png'));
                    elseif count >= 10 & count < 100
                        filename = fullfile(folder,sprintf('%s%d_%s%d%s','test_patch_',k,'00',count,'.png'));
                    elseif count >= 100 & count < 1000
                        filename = fullfile(folder,sprintf('%s%d_%s%d%s','test_patch_',k,'0',count,'.png'));
                    else
                        filename = fullfile(folder,sprintf('%s%d_%d%s','test_patch_',k,count,'.png'));
                    end
                end 
...
writematrix(CSV,'testingPatchLabelsOrdered.csv') 